...
// ── Cron configuration ─────────────────────────────────────────────────────
// The cron prompt is baked into the app as a bundled resource (cron-prompt.txt,
// shipped inside the asar) so it is byte-for-byte identical across every app
// instance. There is NO fallback and NO local-file override — this is the only
// text any user ever sees, whether they read the cron prompt in the Hermes tab
// or open the Hermes dashboard in a browser. To change the prompt, edit
// cron-prompt.txt in the repo and ship a new build.
const CRON_PROMPT_BUNDLED = path.join(__dirname, 'cron-prompt.txt');

function buildCronPrompt() {
  return fs.readFileSync(CRON_PROMPT_BUNDLED, "utf8");
}
